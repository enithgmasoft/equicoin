...
#if defined(NDEBUG)
# error "Equicoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc43d08a37c6ba881c9f890dca09f22c5688452c51f7880279d4e01bc82aa102d");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Equicoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Equicoin Signed Message:\n";
...
{
    // Equicoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Equicoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Equicoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Equicoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Equicoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Equicoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5ad7a53df186438b24a3100af4585889541a7291a6b154e42c0a995291bc6305");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "El Tiempo Colombia, riego de petroleo en barranca, emergencia ambiental";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04d6e2bfe701d0531bcaafb97c6c870884bc96f266c495d5396d3cc10e4487f192e8b0d339577357d6e3969aa38f80af243ab8fda7a56cdb9d29797d91d43d3df6") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1521753634;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087330500;

        if (fTestNet)
        {
            block.nTime    = 1521753536;
            block.nNonce   = 386415432;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x5a49eb4702a9fd007d795e704b9861e14f1ad5ee23d426b93c4c88c31bc7d5b3"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf1, 0xc9, 0xb8, 0xd5 }; // Equicoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EquicoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EquicoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EquicoinMiner(CWallet *pwallet)
{
    printf("EquicoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("equicoin-miner");
    ...
    try { loop {
        ...
        printf("Running EquicoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EquicoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EquicoinMiner, pwallet));
}
